...
| Phase 6 | Conflict Detection | 🟢 已完成 | `06_detect_conflicts.py` + `test_06_conflicts.py` | 1,698 冲突记录 + 16 单元测试 |
| Phase 6.5 | False Positive Filter | 🟢 已完成 | `06_5_filter_false_positives.py` + `_visual_similarity.js` | 视觉相似度像素渲染比对 + 假阳性自动过滤 |
| Phase 7 | Conflict Resolution | 🟡 进行中 | `07_resolve_conflicts.py` + `07_generate_resolver_ui.py` | 审核 UI 增强：false_positive 标注/筛选/排序 + SVG 孔洞渲染修复 |
| Phase 8-9 | Direct Glyf Merge + Font Build | 🟢 已完成 | `08_merge_glyf.py` | 核心合并引擎 + TTF/WOFF2 生成 + CSS/JSON/HTML 输出 |
| Phase 10 | Validation | 🟢 已完成 | `10_validate_render.js` | Playwright + pixelmatch 跨浏览器渲染对比 |
| Phase 11 | Output & Manifest | 🟢 已完成 | `11_generate_manifest.js` | merge_manifest.json + demo_index.html + 报告 |

**脚本统计**：已完成 Phase 1 (3个) + Phase 2 (3个) + Phase 3 (2个) + Phase 4 (2个) + Phase 5 (2个) + Phase 6 (2个) + Phase 7 (2个) + Phase 8-9 (1个) + Phase 10 (1个) + Phase 11 (1个) / 共 19 个
...
| 分类                                      | 数量 | 处理方式                                                         | 项目代码影响             |
| ----------------------------------------- | ---- | ---------------------------------------------------------------- | ------------------------ |
| **自动合并（视觉相似）**                  | 528  | false_positive_merged，保留 canonical name/unicode，合并 sources | 只换 CSS 链接，不改类名  |
| **仍需人工决策**                          | 265  | 需分配 PUA + _vN 后缀 或 keep                                    | 换 CSS 链接 + 可能改类名 |
| **其中：Type A（同 unicode 不同 glyph）** | ~130 | 需在审核 UI 中决策                                               | 保留一个，其余 PUA       |
| **其中：Type B（同 name 不同 glyph）**    | ~135 | 需在审核 UI 中决策                                               | 保留一个，其余 PUA + _vN |

**审核 UI**：`report/conflict_resolver.html`
- Auto-Merged 筛选按钮（528 个已标注绿色）
- 卡片排序：Pending 在前，Auto-Merged 在后
- 进度条：显示 Auto-merged / Pending / Resolved 三态
- SVG 孔洞渲染修复（evenodd fill-rule 跨 contour 生效）

**关键数据（2026-05-14）**：
- 自动合并：**528** 个（视觉相似度 minScore >= 0.90）
- 仍需人工决策：**265** 个（793 审核卡片 - 528 自动合并）
- 视觉相似度阈值：>= 0.90（@napi-rs/canvas 64x64 像素渲染比对）
- **判断逻辑**：`minScore`（所有 pair 最低相似度），要求冲突记录内**所有 variant 两两相似**才自动合并
- 典型修复案例：U+E720（客服耳机 vs 微信气泡）不再错误合并

**已知问题/待办**：
- [ ] 265 个 pending 冲突需在审核 UI 中人工决策（keep/PUA）
- [ ] 决策后运行 `07_resolve_conflicts.py` 重新生成 resolution
- [ ] 继续 Phase 8-11 生成最终字体
