...
| Phase 4 | Geometry Normalization | Python         | `04_normalize_glyphs.py`                                   | UPM=1024、contour 标准化、glyphHash                                       | 🟢 已完成 |
| 5       | Glyph Hash Registry    | Python         | `05_build_registry.py`                                     | 核心数据库、多源合并、alias                                               |
| 6       | Conflict Detection     | Python         | `06_detect_conflicts.py`                                   | Type A/B/C 冲突检测（原始 1,698 条）                                      |
| 6.5     | False Positive Filter  | Python/Node.js | `06_5_filter_false_positives.py` + `_visual_similarity.js` | 视觉相似度像素渲染比对 + 假阳性过滤                                       |
| 6.8     | Type B Resolution      | Python         | `06_8_apply_name_resolution.py`                            | **Type B 全部解决**：保留组合并 + PUA 组重命名 `_v2`                      |
| 7       | Conflict Resolution    | Python         | `07_generate_resolver_ui.py`                               | 审核 UI 生成：支持 `--type-filter` 分阶段审核                             |
...
| Phase 6   | 🟢 已完成（2个脚本：原始 1,698 冲突记录 + 16 单元测试）                                                                           |
| Phase 6.5 | 🟢 已完成（2个脚本：视觉相似度过滤 + 假阳性检测）                                                                                 |
| Phase 6.8 | 🟢 已完成（1个脚本：Type B 全部解决，392→0；PUA 组合并为单 entry，E000+ 分配）                                                    |
| Phase 7   | 🟢 已完成（2个脚本：Type A 决策应用 107条+fallback → Type A=0；07_resolve_conflicts.py 生成 phase7_resolution.json，747 entries） |
| Phase 8-9 | 🟢 已完成（1个脚本：08_merge_glyf.py，746 glyphs 合并输出）                                                                       |
| Phase 10  | 🟢 已完成（1个脚本：10_validate_render.js）                                                                                       |
| Phase 11  | 🟢 已完成（2个脚本：11_generate_manifest.js + 12_package_output.js 整合包）                                                       |

**统计**：Phase 1–11 全部完成，共 21 个脚本。详见 `script.md`。

**最终产出**：`output/` 目录下完整字体包（iconfont_merged.ttf/woff2/css/json + demo_index.html）；`output/package/` 为标准分发包。
