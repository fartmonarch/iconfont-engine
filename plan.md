...
| Phase 5 | Glyph Hash Registry | 🟢 已完成 | `05_build_registry.py` + `test_05_registry.py` | 1346 registry entry + 905 多来源 + 64 最大来源 |
| Phase 6 | Conflict Detection | 🟢 已完成 | `06_detect_conflicts.py` + `test_06_conflicts.py` | 原始 1,698 冲突记录 + 16 单元测试 |
| Phase 6.5 | False Positive Filter | 🟢 已完成 | `06_5_filter_false_positives.py` + `_visual_similarity.js` | 视觉相似度像素渲染比对 + 假阳性自动过滤 |
| Phase 6.8 | Type B Resolution | 🟢 已完成 | `06_8_apply_name_resolution.py` | **Type B 全部解决**（392→0），PUA 组合并为单 entry + 分配新 E000+ PUA |
| Phase 7 | Conflict Resolution | 🟢 已完成 | `07_apply_typea_resolution.py` + `07_resolve_conflicts.py` | Type A 决策应用（107条手动+fallback），Type A→0；phase7_resolution.json 747 entries |
| Phase 8-9 | Direct Glyf Merge + Font Build | 🟢 已完成 | `08_merge_glyf.py` | 核心合并引擎 + TTF/WOFF2 生成 + CSS/JSON/HTML 输出，**746 glyphs** |
| Phase 10 | Validation | 🟢 已完成 | `10_validate_render.js` | Playwright + pixelmatch 跨浏览器渲染对比 |
| Phase 11 | Output & Manifest | 🟢 已完成 | `11_generate_manifest.js` + `12_package_output.js` | merge_manifest.json + demo_index.html + 标准分发包 output/package/ |

**脚本统计**：已完成 Phase 1 (3个) + Phase 2 (3个) + Phase 3 (2个) + Phase 4 (2个) + Phase 5 (2个) + Phase 6 (2个) + Phase 7 (3个) + Phase 8-9 (1个) + Phase 10 (1个) + Phase 11 (2个) / 共 21 个

**待优化**：Phase 4-5 标准化/哈希算法存在假阳性冲突（几何相近但不同的 glyph 被误判为冲突），后续需调整 Phase 4 的 contour 标准化精度或 Phase 5 的 glyphHash 生成策略。当前 1,698 条冲突中大量由此原因导致。

---

### 冲突治理状态（Phase 6.5 + 6.8 + 7，**全部完成**）

| 分类                     | 数量 | 处理方式                                                         | 项目代码影响             |
| ------------------------ | ---- | ---------------------------------------------------------------- | ------------------------ |
| **自动合并（视觉相似）** | 9    | false_positive_merged，保留 canonical name/unicode，合并 sources | 只换 CSS 链接，不改类名  |
| **Type B 已解决**        | 0    | 保留组合并编码 + PUA 组重命名并分配新编码（E000+）               | 换 CSS 链接 + 可能改类名 |
| **Type A 已解决**        | 0    | 107 条手动决策 + fallback 自动 → PUA E000–E0FF 分配              | 保留一个，其余 PUA       |
| **Type C Duplicate**     | 627  | 正常多来源，无需处理                                             | 只换 CSS 链接，不改类名  |

**最终输出（2026-05-15）**：
- Registry：**747 entries**（Type B 解决后，原 1346）
- Phase7 resolution：**747 entries**（全包含 registry）
- 合并字体：**746 glyphs**（1 条 no unicode 跳过）
- PUA 新分配范围：E000–E0FF（~147 个）
- CSS aliases：66 个
- 输出文件：`output/iconfont_merged.{ttf,woff2,css,json}` + `output/demo_index.html` + `output/package/`
...
## 十二、Phase 16 项目替换阶段（🔵 待执行）

### 目标

将各前端项目中的旧 iconfont 独立 CSS 链接替换为统一库地址，并将所有 `icon-xxx` 类名 rename 更新为新的 `icon-xxx_v2` 等名称。

### 工具链

| 已完成 | 工具                                | 作用                                                                                      |
| ------ | ----------------------------------- | ----------------------------------------------------------------------------------------- |
| ✅      | `pipeline/14_generate_replacer.js`  | 已按项目生成 `output/replacers/<proj>_replace.js` + `report/replacement_guides/<proj>.md` |
| ✅      | `output/package/`                   | 标准分发包（iconfont.ttf/woff2/css/json）                                                 |
| ❌      | `pipeline/16_apply_replacements.js` | **待编写**：替换执行器                                                                    |

### 待编写：`pipeline/16_apply_replacements.js`

```
用法：
node pipeline/16_apply_replacements.js \
  --project ai-customer-acquisition \
  --repo-dir D:/work/ai-customer-acquisition \
  --css-cdn-url https://your-cdn.com/iconfont/iconfont.css \
  [--dry-run]
```

功能要求：
1. **CSS 链接替换**：将旧的各项目独立 iconfont CSS URL 替换为 `--css-cdn-url` 指定的统一地址
   - 注意：Phase 1 扫描时模板变量已展开，替换时需还原为模板变量形式（如 `<%= htmlWebpackPlugin.options.path %>`）
2. **类名替换**：`icon-xxx` → `icon-xxx_v2`，精准行号定位 + 全局模式两阶段
3. **--dry-run 预览模式**：只打印操作日志，不写入文件
4. **替换日志**：输出操作了哪些文件、跳过了哪些

### 待项目清单（27 个项目）

| 项目                    | CSS 替换数 | 类名替换数 | 替换指南                                               |
| ----------------------- | ---------- | ---------- | ------------------------------------------------------ |
| ai-customer-acquisition | 2          | 29         | `report/replacement_guides/ai-customer-acquisition.md` |
| app-settlement          | 详见指南   | 详见指南   | `report/replacement_guides/app-settlement.md`          |
| business-tool           | 详见指南   | 详见指南   | `report/replacement_guides/business-tool.md`           |
| ...                     | ...        | ...        | 全部在 `report/replacement_guides/`                    |

执行替换前前置条件：
- [ ] 确定统一 CDN URL（上传 `output/package/` 到 CDN，获取地址）
- [ ] 编写 `pipeline/16_apply_replacements.js`
- [ ] 按项目逐一运行并提交 PR
